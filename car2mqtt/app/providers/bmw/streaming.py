...
# This worker integrates the public BMW CarData device-flow and streaming approach
# described by bausi2k/bmw-python-streaming-mqtt-bridge, adapted for Car2MQTT.
...
class BMWCarDataClient:
    ...
    def refresh_tokens(self) -> bool:
        ...
        self.tokens["refresh_token"] = {
            ...
            "expires_at": self.tokens["refresh_token"]["expires_at"],
        }
        self.tokens["id_token"] = {
            "token": new_tokens["id_token"],
            "expires_at": (now + __import__("datetime").timedelta(seconds=int(new_tokens.get("expires_in", 3600)))).replace(microsecond=0).isoformat(),
        }
...
class BMWStreamWorker:
    def __init__(self, vehicle, mqtt_settings, state_store, local_mqtt_client_factory, on_payload: Callable[[str, Dict[str, Any]], None]):
    ...
    def _run(self):
        ...
        if not token_file.exists():
            return
        self.local_client = self.local_mqtt_client_factory(self.mqtt_settings)
